...
EOF
    ...
    print OUT "        public static readonly ushort[] CombiningClasses = new ushort[]\n        {\n";
    for ($count = 0; $count <= $last; $count += 256)
    {
	print OUT ",\n" if $count > 0;
	print OUT "            ", $row[$count / 256];
	...
	if (defined $decompositions[$count])
	{
		...
		$compat_offset = (length $string) / 6;
		$string .= $compat_decomp;
	    }

            if (!defined($decomp_offsets{$string})) {
                $decomp_offsets{$string} = $decomp_string_offset;
                $decomp_string .= "\n            ".$string."0x00, 0x00, /* offset ".
                    $decomp_string_offset." */";
                $decomp_string_offset += ((length $string) / 6) + 2;
	    
                $bytes_out += (length $string) / 6 + 2; # "\x20"
            }
            
            my $co = ($compat_offset == 0xff) ? $canon_offset : $compat_offset;
	    
            printf OUT qq(            new Decompose('\\x%04x', %d)), 
                $count, $co + $decomp_offsets{$string};
	    $bytes_out += 6;
    ...
    printf OUT "        public static readonly byte[] Expansion = new byte[]\n        {%s\n        };\n", $decomp_string;
...
sub make_decomp
{
    ...
    {
	$result .= sprintf "0x%02x, 0x%02x, ", $iter / 256, $iter & 0xff;
    }
